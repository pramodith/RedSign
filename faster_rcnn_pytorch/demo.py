...
root_dir="faster_rcnn_pytorch"

with open("data/VOCdevkit2007/VOC2007/Red_Annotations/gt.txt", 'r') as f:
    bbox_locations = {}
    lines = f.readlines()
    for line in lines:
        columns = line.split(";")
...
def parse_args():
  ...
  parser.add_argument('--image_dir', dest='image_dir',
                      ...
                      default="data/VOCdevkit2007/VOC2007/Red_Data/")
  ...
  while (num_images >= 0):
      ...
      for j in xrange(1, len(pascal_classes)):
          ...
          if inds.numel() > 0:
            ...
            pred_boxes= np.asarray([[int(np.round(x)) for x in cls_dets[i, :4]] for i in range(np.minimum(10,cls_dets.shape[0]))])
            score = np.asarray([cls_dets[i,-1].cpu().numpy() for i in range(np.minimum(10,cls_dets.shape[0]))])
            print(score)
            indixes=np.where(score>0.7)
            print(indixes)
            pred_boxes=pred_boxes[indixes]
            try:
                g_loc = bbox_locations[im_file.split("/")[-1]][:]
                print(pred_boxes.shape)
                print(g_loc)
                for box in pred_boxes:
                    flag = 0
                    for ind, gt_box in enumerate(g_loc):
                        if bbox_iou(box, gt_box) > 0.5:
                            del g_loc[ind]
                            flag = 1
                            tp += 1
                            break
                ...
                for box in bbox_locations[im_file.split("/")[-1]][:]:
                    flag = 0
                ...
                pass
            if vis:
              im2show = vis_detections(args.image_dir+"/"+imglist[num_images],im2show, pascal_classes[j], cls_dets.cpu().numpy(), 0.7)
